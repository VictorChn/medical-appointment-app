...
<aside id="top-bar-sidebar" class="fixed top-0 left-0 z-40 w-64 h-full transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
   <div class="h-full px-3 py-4 overflow-y-auto bg-neutral-primary-soft border-e border-default">
      <a href="/" class="flex items-center ps-2.5 mb-5">
         <img src="{{ asset('images/mini_oguri.webp') }}" class="h-6 me-3" alt="Healtify Logo" />
      </a>
      <ul class="space-y-2 font-medium">
         ...
         <li>
            ...
            @else
               <a href="{{ $link['href'] }}" class="flex items-center w-full p-2 text-base text-gray-900 {{ $link['active'] ? 'bg-gray-100' : '' }}">
                  <span class="w-6 h-6 inline-flex items-center justify-center text-gray-500">
                     <i class=" {{ $link['icon'] }} " ></i>
                  </span>
